...
function ViewPage() {
  ...
  const viewData=task.filter((item)=>{
  ...
  function goToEdit(){
    navigate(`/form/${id}?mode=edit&from=view`,{replace: true})
  }
  return (
      ...
      <div className="bg-slate-100  card text-gray-500  font-medium mt-7 p-5 xs-sm:p-10 rounded-md flex flex-col gap-4">
         ...
         <div>
            ...
            <div className="px-4 py-2 mt-1 border-y-2 border-slate-200 flex flex-col gap-1">
                ...
                <p>Due Date:&nbsp;<span className="whitespace-nowrap text-center font-normal">{new Date(viewData.date).toLocaleDateString("en-IN")}</span></p>
                <p>Task Priority:&nbsp;   
                {
